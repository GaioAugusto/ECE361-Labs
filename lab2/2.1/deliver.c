...
// Check number of arguments passed in the command-line
    ...
    // Start timer
    struct timespec start, end; // store timestamps
    clock_gettime(CLOCK_MONOTONIC, &start); // gets start time

    // Send message to server (same code as last lab)
    ...
    // Stop time
    clock_gettime(CLOCK_MONOTONIC, &end); // captures end time

    buffer[bytesRecv] = '\0'; // Null terminate

    // Calculate elapsed time (convert from nano to seconds)
    double rtt = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9; // difference between end and start time
