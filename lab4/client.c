...
int main(int argc, char *argv[])
{
    ...
    {
        // Login prompt
        printf("Please enter your command in the format: /login <clientID> <password> <serverIP> <server-port>\n");
        ...
        char command[16], clientID[64], password[64], serverIp[64];
        int serverPort;
        int scanned = sscanf(userInput, "%s %s %s %s %d", command, clientID, password, serverIp, &serverPort);

        // We expect 5 tokens total: "/login", clientID, password, serverIP, server-port
        if (scanned != 5 || strcmp(command, "/login") != 0)
        {
            fprintf(stderr, "Invalid command. Expected format:\n"
                            "/login <clientID> <password> <serverIP> <server-port>\n");
        ...
        // Create struct
        struct message login = create_message(LOGIN, clientID, password);

        // Serialize struct
        char serialized_login[1024];
        serialize_message(&login, serialized_login, sizeof(serialized_login));
        ...
        // Receive response
        char buffer[1024];
        struct message login_response;
        receive_message(sockfd, buffer, sizeof(buffer));
        login_response = deserialize_message(buffer);
        ...
        {
            printf("Login failed\n");
            printf("%s\n", (const char *)login_response.data); // reason for failure
            close(sockfd);
            continue;
        }
        ...
        while (1)
        {
            ...
            else if (strncmp(userInput, "/joinsession ", 13) == 0)
            {
                // Extract sessionID
                char sessionID[MAX_DATA];
                strcpy(sessionID, userInput + 13);
            ...
            else if (strncmp(userInput, "/createsession ", 15) == 0)
            {
                // Extract sessionID
                char sessionID[MAX_DATA];
                strcpy(sessionID, userInput + 15);
            ...
            else if (strcmp(userInput, "/list") == 0)
            {
                ...
                receive_message(sockfd, buffer, sizeof(buffer));
                struct message msg = deserialize_message(buffer);
                printf("List of clients and sessions on server: %s\n", msg.data);
            }
            ...
            else
            {
                ...
                struct message response = create_message(MESSAGE, clientID, userInput);
                char serialized[2048];
                serialize_message(&response, serialized, sizeof(serialized));
                send_message(sockfd, serialized);
            }
